...
> **Read the split before you read the scores.** LongMemEval ships the same 500 questions in three haystacks: `oracle` (evidence sessions only), `_s` (~40 sessions, ~115k tokens), `_m` (~500 sessions). The gap is enormous — the benchmark's own authors measure GPT-4o at **0.924 on `oracle` versus 0.640 on `_s`**, a ~28-point drop on *identical questions*. **These numbers are `oracle`.** They are therefore **not comparable** to any vendor number reported on `_s`, and the table below must never be read against, say, Zep's published 71.2% or Mem0's 94.4% — both of which are on different or unnamed splits. An `_s` run is the honest next milestone and is not yet done.
...
That last row is the number most benchmark tables leave out, and it's the one that makes the rest interpretable. It hands the answerer the gold sessions **directly, with retrieval switched off** — so it is the practical maximum any memory system can reach against this data, answerer, and judge. AgentMem OS reaches **92% of that ceiling**. A memory system's job is to find the right memory; measuring it without knowing what perfect retrieval would score tells you nothing about whether a gap is the system's fault or the benchmark's. (It is an empirical bound, not a mathematical one: the oracle's context is capped at 24k characters, so questions with very large haystacks can lose evidence to truncation.)
