"""Chunk(turn)-level Recall@k for the production retrieval stack. $0.
...
benchmarks/recall_at_k_chunklevel_results.json
"""
import json
import sys
import time
...
def main():
    ...
    for i, q in enumerate(items):
        ...
        row = {"question": q.question[:60], "type": q.question_type,
               "n_gold": len(golds)}
        for k in KS:
            top = set(ranked_turn_sessions[:k])
            a = bool(golds & top)
            al = golds <= top
            any_hit[k] += a
            all_hit[k] += al
    ...
    out = {
        ...
        "retriever": "MultiVectorRetriever (multilingual-e5-small dense "
                     "+ TF-IDF, RRF k=60), CHUNK-LEVEL: top-k TURNS, "
                     "recall counts sessions represented among them",
        "any_gold_recall": {f"@{k}": round(any_hit[k] / n, 4) for k in KS},
        "all_gold_recall": {f"@{k}": round(all_hit[k] / n, 4) for k in KS},
        "per_question": per_q,
    }
    path = HERE / "recall_at_k_chunklevel_results.json"
