"""
Retrieval evaluation metrics — ported from X-MemoryArch
(RetrievalEngine/app/services/benchmark/eval_metrics.py), a separate
project by the same author with an already-tested LoCoMo/LongMemEval
benchmark harness. See LAUNCH_ROADMAP.md Phase 1 Group D and
...
All functions accept:
  retrieved:  ordered list of memory/session IDs (highest rank first)
  relevant:   set or list of gold-label IDs
  k:          cutoff (default matches function name)

Note: these are RETRIEVAL RECALL metrics — did the relevant item land in the
top-K? — not QA accuracy (retrieve -> generate -> judge correctness). They
are a different, easier metric family than what Mem0/Zep/Honcho publish;
see benchmarks/qa_accuracy_eval.py (Phase 1 Group E) for the comparable one.
Recall typically runs 20-30 points higher than QA accuracy on the same data
— do not compare a recall number here against a competitor's QA-accuracy
claim as if they were the same thing.

Usage:
  from agentmem_os.benchmarks.eval_metrics import recall_at_k, mrr_at_k, ndcg_at_k, evaluate
"""
...
def ndcg_at_k(retrieved: list[str], relevant: list[str] | set[str], k: int) -> float:
    ...
    IDCG@k = sum(1 / log2(i + 1))      for i in 1..min(|relevant|, k)
    """
    relevant_set = set(relevant)
    if not relevant_set:
        return 0.0
...
@dataclass
class EvalReport:
    """Aggregated evaluation results across a set of queries for one dataset/system."""
    scenario_name: str
    ...
    def summary(self) -> str:
        """Human-readable one-paragraph score report. No pass/fail targets —
        this project hasn't validated a target threshold the way
        X-MemoryArch's own internal SLAs did; report raw numbers only."""
        return (
            f"=== {self.scenario_name} ({self.n} queries) ===\n"
            ...
            f"  MRR@10={self.mrr_at_10:.3f}  NDCG@10={self.ndcg_at_10:.3f}\n"
...
class BenchmarkSuite:
    """Aggregated results across all scenarios/datasets in a benchmark run."""
